...
sealed class MSTransform : MSComponent<Transform>
{
    ...
    protected override Boolean UpdateMSComponent()
    {
        ...
        RotX = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.X);
        RotY = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.Y);
        RotZ = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.Z);
        
        ScaleX = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.X);
        ScaleY = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.Y);
        ScaleZ = MSEntity.GetMixedValue(SelectedComponents, x => x.Position.Z);
